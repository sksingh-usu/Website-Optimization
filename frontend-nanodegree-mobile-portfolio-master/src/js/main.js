...
var pizzaElementGenerator = function(i) {
  ...
  /* Here I have commented the height and width styling to prevent the CSSOM styling on each iteration. */
...
var currentPizzaClass="randomPizzaContainer";
...
window.performance.mark("mark_start_resize");   // User Timing API function  
  ...
  function changeSliderLabel(size) {
    ...
    switch(size) {
      case "1":
        ...
        return "smallPizzaContainer";
      case "2":
        document.querySelector("#pizzaSize").innerHTML = "Medium";
        return "mediumPizzaContainer";
      case "3":
        document.querySelector("#pizzaSize").innerHTML = "Large";
        return "largePizzaContainer";
      default:
        console.log("bug in changeSliderLabel");
    }
  }
  
  var sizeClass = changeSliderLabel(size);
  changePizzaSizes(sizeClass);
  ...
  /*
	Here I leveraged the css to reduce the calculations overhead due to width calculations on every iteration while resizing the pizza.
  */
  function changePizzaSizes(sizeClass) {
	  var pizzaList = document.getElementsByClassName(currentPizzaClass);
	currentPizzaClass = sizeClass;
	var i = pizzaList.length-1;
    while(i>=0){
		  pizzaList[i].className = sizeClass;
		  i--;
	} 
}
...
function updatePositions() {
  ...
  var scroll_factor = document.body.scrollTop / 1250;
  var items = document.getElementsByClassName('mover');
  var phase_arr =[];
  /* Instead of calculating Math.sin on each iteration I have stored the factors in 
	 phase_arr and this helps optimizing the code for loop speed*/
  for(var i=0; i<5; i++){
	  phase_arr.push(Math.sin((scroll_factor) + (i % 5)));
  }
  
  for (var i = 0; i < items.length; i++) {
    var phase = phase_arr[i%5]; // Getting the phase from already calculated phase array
    items[i].style.transform = 'translate(' + 100 * phase + 'px)'; //(' + items[i].basicLeft + 100 * phase + 'px)'; //= translateX(items[i].basicLeft + 100 * phase + 'px');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var rows = window.screen.height / (IMAGE_HEIGHT+IMAGE_OFFSET);
  var totalPizza = rows * cols;
  var movingPizzas = document.getElementById("movingPizzas1");
  
  
	/* Restrict the loop to fill the page only once
		I have seen multiple pizzas being painted redundantly so instead of 200 pizzas now the number will be calculated based on height and width of the screen.
	*/
  for (var i = 0; i < totalPizza; i++) {
	...
	/* Removed the styling here and resized the image to fit well on the screen
		That helps minimizing the number of recalculations involved.
		
    elem.style.height = "100px";
	elem.style.width = "73.333px";
	*/
	
	// Instead of setting the basic left property, set the true left property and then use transform function to translate the pizzas in updatePositions() code.

    elem.style.left = (i % cols) * s + 'px';
    elem.style.top = (Math.floor(i / cols) * s) + 'px';
    movingPizzas.appendChild(elem);
  }
  /* Removed the updatePosition() function call which helped removing the recalculation overhead on document Node */
});
